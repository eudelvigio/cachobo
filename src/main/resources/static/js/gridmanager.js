...
/*------------------------------------------ Editable Regions ---------------------------------------*/
        ...
        gm.addEditableAreaClick = function(container, btn) {
          ...
          .before(elem = $('<div>').addClass(gm.options.gmEditRegion+' '+gm.options.contentDraggableClass)
            .append(gm.options.controlContentElem+'<div class="'+gm.options.gmContentRegion+'" onclick="if (this.innerHTML == \'...\') this.innerHTML=\'\';">...</div>')).before(cTagClose).prev().before(cTagOpen);
...
/*------------------------------------------ BTNs ---------------------------------------*/
        ...
        gm.buttonFactory=function(btns){
          ...
          $.each(btns, function(i, val){
            ...
            val.onclick = (typeof val.onclick === 'undefined')? '' : val.onclick;
            buttons.push("<" + val.element +" title='" + val.title + "' onclick='"+val.onclick+"' class='" + val.btnClass + "'><span class='"+val.iconClass+"'></span>&nbsp;" + val.btnLabel + "</" + val.element + "> ");
          });
          return buttons.join("");
        };
                ...
                switch (gm.options.rte) {
                    ...
                    case 'ckeditor':
                      ...
                      for (var i in CKEDITOR.instances) {
                    	 window.CKEDITOR.instances[i].addCommand("customElementor", {
                    		 exec: function (edt) {
                    			 gm.options.ckeditor.handlerCustomElementor(edt);
                    		 }
                  		 });
                  		 window.CKEDITOR.instances[i].ui.addButton("Elements", {
                  			 label: "Elementator",
                  			 command: "customElementor",
                  			 toolbar: 'insert',
